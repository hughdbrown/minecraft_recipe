...
function getDisplayNameFromLanguage(identifier, type) {
    ...
    const typePrefix = type === 'item' ? 'item' : 'block';

    // Format 1: item.namespace:item_name.name
    const key1 = `${typePrefix}.${identifier}.name`;

    // Format 2: item.namespace.item_name.name (colon replaced with dot)
    const key2 = `${typePrefix}.${identifier.replace(':', '.')}.name`;

    // Search all language files
    for (const langData of Object.values(editBlocksItemsLanguageFiles)) {
        if (langData.entries.has(key1)) {
            return langData.entries.get(key1);
        }
        if (langData.entries.has(key2)) {
            return langData.entries.get(key2);
        }
    }
...
async function scanBlocksAndItems() {
    ...
    console.log('Looking for language files...');
    if (resourcePack) {
        console.log('Resource pack found:', resourcePack);
        for (const [path, zipEntry] of Object.entries(editBlocksItemsZip.files)) {
            if (zipEntry.dir) continue;
            const lowerPath = path.toLowerCase();

            // Look for .lang files in texts folder
            if (lowerPath.includes('/texts/') && lowerPath.endsWith('.lang')) {
                try {
                    const content = await zipEntry.async('string');
                    ...
                    console.log(`Found language file: ${path} with ${entries.size} entries`);
                    console.log('Sample entries:', Array.from(entries.entries()).slice(0, 5));
                } catch (error) {
                    console.error(`Error parsing language file ${path}:`, error);
                }
            }
        }
    } else {
        console.log('No resource pack found');
    }
    console.log('Total language files loaded:', Object.keys(editBlocksItemsLanguageFiles).length);
...
async function applyDeletion(identifier) {
    ...
    const typePrefix = item.type === 'item' ? 'item' : 'block';
    const key1 = `${typePrefix}.${identifier}.name`;
    const key2 = `${typePrefix}.${identifier.replace(':', '.')}.name`;
    ...
    for (const [langPath, langData] of Object.entries(editBlocksItemsLanguageFiles)) {
        ...
        if (langData.entries.has(key1)) {
            langData.entries.delete(key1);
            modified = true;
        }
        if (langData.entries.has(key2)) {
            langData.entries.delete(key2);
            modified = true;
        }
...
function updateLanguageFiles(originalIdentifier, newIdentifier, newDisplayName, itemType) {
    const typePrefix = itemType === 'item' ? 'item' : 'block';

    // Try different key formats for the original identifier
    const oldKey1 = `${typePrefix}.${originalIdentifier}.name`;
    const oldKey2 = `${typePrefix}.${originalIdentifier.replace(':', '.')}.name`;

    // New keys if identifier changed
    const newKey1 = `${typePrefix}.${newIdentifier}.name`;
    const newKey2 = `${typePrefix}.${newIdentifier.replace(':', '.')}.name`;
    ...
    for (const [langPath, langData] of Object.entries(editBlocksItemsLanguageFiles)) {
        ...
        if (langData.entries.has(oldKey1)) {
            keyFormat = 1;
        } else if (langData.entries.has(oldKey2)) {
            keyFormat = 2;
        }

        if (keyFormat) {
            const oldKey = keyFormat === 1 ? oldKey1 : oldKey2;
            const newKey = keyFormat === 1 ? newKey1 : newKey2;
